...
| `tui.py` + `ui/` | **Interactive TUI (Sprint J, overhauled in Sprint K)** — Matrix-green (`#048c04`) terminal app on top of the CLI. Sprint K rebuilt every menu on raw `prompt_toolkit.Application` instances (not the shortcuts, which leak a light theme) for full theme control, with uniform controls (Space toggle / Enter OK / Esc back / Q quit-with-confirm / A·N all·none). `ui/theme` exposes the exact phosphor palette + ANSI; `ui/menus` adds paste-your-cookie (TextArea + `validate_cookie_json`, saved to `.cookies/{source}.json`), the always-visible Dateas 3+3 options, and the live multi-query (`--`) input; `ui/progress` renders the live scraping box and the in-terminal `render_results_table`. `tui.py` runs each `--`-split query independently, dedups across queries, tags leads with their `Query`, and names the workbook after the query (`exporters.csv_exporter.build_output_filename`). Falls back to a CLI hint when there is no TTY. `main.py` CLI is unchanged (adds `parse_multi_query`). |
| Cookies | `utils/cookies.load_cookies` reads `.cookies/{source}.json`; Instagram/Facebook/Twitter/MercadoLibre pass it to `StealthyFetcher.fetch(cookies=…)`, LinkedIn flattens it into a curl_cffi name→value dict. `.cookies/` is git-ignored. |
| Performance | Google Maps detail fetch uses `network_idle=False` and `get_optimal_workers(multiplier=8, cap=64)`; `async_pipeline` runs `max_concurrent=5` with `as_completed` and `distribute_limit` (total limit split across sources, min 3 each); `email_scraper` is settings-driven (`EMAIL_SCRAPING_TIMEOUT=3`, `MAX_PAGES=1`, `MAX_CONCURRENT=10`) with an in-session domain→email cache. StealthyFetcher scrapers run `network_idle=False`; social scrapers (ig/fb/tw/ml) at `timeout=12000`, CF-bound AR scrapers keep 60000. |
| Email scraping (Sprint L) | `settings.EMAIL_SCRAPING_ENABLED` gates the website-email step. `pipeline.should_email_scrape(sources, limit, user_disabled)` auto-disables it for >3 sources or limit >30; CLI `--no-email-scraping`, TUI "Fast/Complete" speed option. |
| Live feed (Sprint L) | `ui/progress.ScrapingState` (thread-safe) is fed by `on_lead` / `on_source_status` callbacks threaded through `pipeline.run` and `scrape_sources`. `run_with_progress` redraws ~5×/s: per-source bars, elapsed timer, last-8-leads feed, and a non-alarming yellow notices panel (`StateLogHandler` routes WARNING+ logs there). `tui` silences terminal logging so the UI owns the screen. `utils/session_cache.SessionCache` reuses leads across queries within a session. |
| Cookies (Sprint L) | `utils/cookie_detector` auto-detects browser sessions (Chrome/Edge/Firefox via `browser-cookie3`, graceful when absent). The TUI cookie step offers auto-detect / manual paste / skip, with manual fallback on failure. |
| Results table (Sprint L) | `render_results_table` always renders when leads exist, with data-adaptive columns (`_detect_available_columns`: name/source always, phone/email/website + cuit/dni/locality when present). `print_results` shows a per-source breakdown and a yellow note when registry sources (Dateas/Top Doctors) dominate. |
| Performance max (Sprint M) | `utils/hardware.detect_hardware` (psutil) sizes workers to CPU+RAM (`recommended`=half of `min(64, logical×8)`, RAM-capped, ≥4); `utils/browser_config.get_stealth_fetch_kwargs` centralizes fetch kwargs (maps the `block_resources` toggle to Scrapling's `disable_resources` + `blocked_domains` set + `block_ads`); every StealthyFetcher/DynamicFetcher scraper now uses it. Google Maps detail fetches block resources at `timeout=10000` and size workers via `settings.WORKERS` or the hardware profile. `config.settings` gains `WORKERS/NETWORK_IDLE/BLOCK_RESOURCES/BROWSER_REUSE/MAX_CONCURRENT_SOURCES` (env-backed). `utils/browser_pool` is a stub for future Playwright reuse. TUI adds a performance screen (hardware readout, workers auto/manual, fast/complete load, resource blocking) and a RENDIMIENTO/PERFORMANCE block in the summary. |
| Tests | 232/232 passing. Sprint M added 11 (hardware ×4, browser_config ×5, email-domain-skip ×1, distribute-min ×1). Sprint L added 16 (`distribute_limit` ×3, `should_email_scrape` ×4, `ScrapingState` ×2, `SessionCache` ×2, `on_lead` ×1, cookie detector ×5, dynamic-columns table ×1 — minus overlaps counted in test_tui/test_performance/test_cookie_detector). Sprint K added 12. Sprint J 10. Sprint I 8. |
| `exporters/csv_exporter.py` | Done — styled `.xlsx`; **dynamic columns**: appends DNI/CUIT/Edad/Provincia/Localidad/Tipo when the result set has `dateas` leads (CUIT/DNI-aware totals row), and a `Query` column for multi-query runs. |
